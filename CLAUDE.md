...
- **Updater endpoint** — `plugins.updater.endpoints` in `tauri.conf.json` is set to `https://pagedge.com/api/update/{{target}}/{{arch}}/{{current_version}}`. **This endpoint does not exist yet** (Step 26/32) — the app will simply fail the update check silently until it's live.
- **Updater signing key — NOT generated/committed.** `plugins.updater.pubkey` in `tauri.conf.json` is a placeholder (`"REPLACE_WITH_GENERATED_PUBLIC_KEY"`). Before the updater can actually work, run `pnpm tauri signer generate -w ~/.tauri/pagedge-updater.key` (outside the repo), paste the printed public key into `pubkey`, and store the private key + password as the `TAURI_PRIVATE_KEY` / `TAURI_KEY_PASSWORD` GitHub secrets used by the release workflow. **Never commit the private key.**
...
- **`lib/supabase.ts`** — two clients: `supabaseAdmin` (service-role key, bypasses RLS, server-only — used for `auth.admin.createUser` and reading/writing `profiles`) and `supabaseAnon` (anon key — used for the actual password/refresh grants and for verifying a bearer token). **Never** the service-role key client for anything reachable from a token the client controls.
...
- **`GET /auth/me`** — returns `{ user_id, email, tier, ai_calls_this_month, calls_remaining }`; `calls_remaining` is `null` for `tier: 'pro'`, else `max(0, 30 - ai_calls_this_month)`. **Lazy monthly reset** — not a cron job — if `ai_calls_reset_at <= now()`, the route itself zeroes `ai_calls_this_month` and bumps `ai_calls_reset_at` to the 1st of next UTC month before computing the response. No route in this step increments `ai_calls_this_month` — that belongs to whichever future step wires AI calls through the backend for quota enforcement; this step only lays the account/quota *read* path.
- **Manual setup required (not automatable from this session)** — create the Supabase project via dashboard, run the migration SQL, copy `NEXT_PUBLIC_SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY`/`SUPABASE_ANON_KEY` into `.env.local` and as Vercel project env vars, then `vercel deploy --prod`. Full steps in `pagedge-backend/README.md`. Verified locally: `npm run build` succeeds with placeholder env vars (all 4 routes compile as dynamic functions) — never actually hit a live Supabase project.
...
- **Settings panel** — `.settings-account` block (email + "Sign out" link-button) at the top of `settings-body`, above the AI provider fields. `"AI Calls: {used} / 30 this month"` shown only when `user.tier === 'free'` (derived as `30 - callsRemaining`, the exact inverse of the backend's `calls_remaining` formula); hidden entirely for `'pro'`.
