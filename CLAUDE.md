...
- **Settings panel** — `.settings-account` block (email + "Sign out" link-button) at the top of `settings-body`, above the AI provider fields. As of Step 32: free users see `"Free Plan — {used} / 30 AI calls this month"` (derived as `30 - callsRemaining`) + an "Upgrade to Pro →" link; pro users see `"Pro Plan — Unlimited AI"` + a "Manage subscription" link. The provider/model/base-URL/API-key fields are now hidden by default behind a "Use my own AI provider" toggle (see Step 32 below).
