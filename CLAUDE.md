...
- **Updater endpoint** — `plugins.updater.endpoints` in `tauri.conf.json` points at `https://pagedge-backend.vercel.app/api/update/{{target}}/{{arch}}/{{current_version}}`, **not** GitHub directly. The `Pagedge` repo is private, so an unauthenticated request (which is all the Tauri updater can make — it has no GitHub credentials) 404s against GitHub Releases even for a fully published release; confirmed by testing the raw asset URL with plain `curl`. The backend route (`pagedge-backend/app/api/update/[target]/[arch]/[version]/route.ts`, plus `.../update/download/[tag]/[filename]/route.ts`) uses a server-side `GITHUB_RELEASES_TOKEN` (fine-grained PAT, Contents:Read-only, scoped to just this repo) to fetch `latest.json` and resolve installer downloads to GitHub's short-lived signed CDN URLs, redirecting the client there so multi-ten-MB installers never get proxied through the Vercel function itself. See `lib/github.ts` in the backend repo.
- `tauri-action` auto-generates and signs `latest.json` as a release asset on every tag (using `TAURI_SIGNING_PRIVATE_KEY`/`TAURI_SIGNING_PRIVATE_KEY_PASSWORD`, wired in `release.yml`). **Requires `bundle.createUpdaterArtifacts: true`** in `tauri.conf.json` (Tauri v2's explicit opt-in for producing signed updater artifacts at build time) — the `v0.1.0`/`v0.1.1` GitHub releases predate this flag and have no `latest.json`/`.sig` assets at all; only releases built *after* this flag was added carry a real updater manifest. **Caveat:** `release.yml` creates the GitHub Release as a **draft** on purpose (a chance to review before going live) — the backend proxy calls `GET /repos/.../releases/latest`, which only ever returns published releases, so a new version isn't visible to the in-app updater until the draft is manually published on GitHub.
...
- **`App.tsx` gating (updated post-Step 26)** — auth is now a soft gate, not a hard wall: `authLoading` → bare `.app-shell` (no flash); otherwise the full app tree (`LibrarySidebar`/`MainArea`/`RightPanel`/etc.) always renders regardless of `isAuthenticated`, and `AuthModal` only appears as a dismissible overlay (`authPromptOpen`) when a specific action calls `requireAuth(reason, onSuccess)` (AI calls, Global Chat, sync, etc.) — local reading/highlighting/notes work fully signed-out. `initAuth()` and `checkForUpdates()`/`loadPdfs()`/`loadAiSettings()` all fire from the same mount effect.
...
- **Forgot / reset password** — `AuthModal` gained `forgot-password` and `reset-password` modes alongside `signin`/`signup`/`verify-email`. Sign-in tab has a "Forgot password?" link → `forgot-password` mode collects an email and calls `POST /auth/forgot-password` (backend: `pagedge-backend/app/api/auth/forgot-password/route.ts`, rate-limited via `lib/rateLimit.ts`, always returns `200` regardless of outcome — never reveals whether the email exists). The desktop `reset-password` mode (via `pagedge://auth/reset` + `passwordResetToken` in the store, mirroring `authTokenError`/`verify-email`) is a same-device convenience path — see the cross-device bridge page below for the primary path.
- **Cross-device confirm/reset bridge page** — `pagedge-web/confirm.html` (static, no framework, matches the landing page's paper/ink/Fraunces+Inter design tokens). Both Supabase's signup-confirmation link and the password-recovery link (`forgot-password`'s `resetPasswordForEmail(email, { redirectTo: 'https://pagedge.com/confirm.html' })`) redirect here instead of straight to a `pagedge://` deep link — a raw custom-scheme link fails silently if the confirmation email is opened on a phone with no desktop app installed. The page reads Supabase's `#access_token=...&type=...` hash fragment client-side (fragments never touch the server) and branches on `type`: **`signup`** → shows "Email confirmed" (Supabase already confirmed the account server-side before this redirect, regardless of device) and attempts `window.location.href = 'pagedge://auth/confirm#'+hash` as a same-device convenience, falling back after ~1.2s to plain-text instructions to sign in on the original device — no dead end if the deep link can't fire. **`recovery`** → renders an inline password form on the page itself and `fetch()`s `POST {API_BASE_URL}/auth/reset-password` directly over HTTPS, so a password reset can be completed entirely from whatever device the email was opened on, no desktop app required at all. **Requires two one-time Supabase dashboard changes**: (1) add `https://pagedge.com/confirm.html` to Authentication → URL Configuration → Redirect URLs (alongside the existing `pagedge://auth/confirm`), (2) change the default signup-confirmation redirect (Authentication → URL Configuration → Site URL / email template redirect) from `pagedge://auth/confirm` to `https://pagedge.com/confirm.html` — `admin.createUser()` in the signup route doesn't take a per-call `redirectTo`, so this one is dashboard-config-only, unlike `forgot-password`'s code-level `redirectTo`.
- **Confirmation-email branding — not yet fixed.** Supabase's default auth emails (signup confirmation, password recovery) are sent from Supabase's own address with Supabase's default template, not from a Pagedge-branded sender — even though Resend is already wired up for waitlist/feedback email (`RESEND_API_KEY`/`FROM_EMAIL` in the backend). Fixing this is dashboard-only, no code change possible from this repo: (1) Supabase dashboard → Project Settings → Auth → SMTP Settings → enable custom SMTP using Resend's SMTP relay (host `smtp.resend.com`, port `587`, user `resend`, password = the `RESEND_API_KEY` value, sender = `FROM_EMAIL`); (2) Supabase dashboard → Authentication → Email Templates → edit the "Confirm signup" and "Reset password" templates' subject/body to Pagedge branding/copy. Until both are done, these two auth emails still look and come from generic Supabase, unlike the Resend-sent waitlist/feedback emails.
